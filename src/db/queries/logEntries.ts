...
export async function createLogSession(input: CreateLogSessionInput): Promise<LogEntry[]> {
...
export async function updateLogEntry(id: string, changes: UpdateLogEntryInput) {
  await db.logEntries.update(id, { ...changes, updatedAt: new Date().toISOString() })
}
